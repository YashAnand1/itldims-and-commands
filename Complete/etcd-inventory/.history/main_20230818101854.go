...
func listAll(w http.ResponseWriter, r *http.Request) {
	...
	for _, kv := range response.Kvs {
		...
		// Get revision values for the previous two revisions
		revisions, err := getRevisionValues(etcdClient, key, []int64{kv.ModRevision - 1, kv.ModRevision - 2})
		if err != nil {
		...
		if len(revisions) > 0 {
			...
			for _, revValue := range revisions {
				fmt.Fprintf(w, "Value: %s\n", revValue)
			}
		} else {
			fmt.Fprintf(w, "Previous Value: No previous revisions\n")
		}
