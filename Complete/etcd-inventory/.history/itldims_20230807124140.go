	...
	get = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for key, value := range data {
				...
				if len(args) == 1 {
					// If only one argument is provided, search for it in both keys and values
					if !strings.Contains(key, "data") && !strings.Contains(value, "{") && !strings.Contains(value, "{") &&
						strings.Contains(value, args[0]) && strings.Contains(key, args[1] || strings.Contains(value, args[0]) && strings.Contains(key, args[1]) {
						fmt.Printf("%s\n%s\n\n", key, value)
					}
				} else if len(args) == 2 {
					// If two arguments are provided, search for them separately in keys and values
					if !strings.Contains(key, "data") && !strings.Contains(value, "{") && !strings.Contains(value, "{") &&
						strings.Contains(key, args[0]) && strings.Contains(value, args[1] || strings.Contains(value, args[0]) && strings.Contains(key, args[1]) {
						fmt.Printf("%s\n%s\n\n", key, value)
					}
				}
			}
