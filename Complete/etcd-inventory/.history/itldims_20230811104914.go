...
- itldims get <Server IP> <Server Type> 	| Display Attribute values of a specific Server
	...
	Run: func(cmd *cobra.Command, args []string) { //This function is to be executed when 'get' subcommand is run
		...
		if len(args) == 1 && (args[0] == "servers") || (args[0] == "servers-types") { //If a single argument made is called servers, this will run
			uniqueIPs := make(map[string]string) // maps store only unique keys and not duplicates.WE ARE ONLY DEFINING WHAT
			...
			for key := range parseKeyValuePairs(data) {
				...
				if args[0] == "servers" {
					serverType = splitKey[2] // Extract the server type
				}
