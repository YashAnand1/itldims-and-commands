		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if response.StatusCode == http.StatusOK {
				...
				if attributeValue != "" {
					...
					log.Printf("Attribute value not found for server IP %s and attribute %s.\n", aip, attribute)
				}
			} else {
				log.Fatalf("Failed to fetch the attribute value for server IP %s and attribute %s. Status code: %d", aip, attribute, response.StatusCode)
			}
		},
	}
)
