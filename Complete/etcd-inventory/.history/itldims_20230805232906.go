	...
	get = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for key, value := range data {
				...
				if strings.Contains(value, args[0]) && strings.Contains(key, args[1]) || strings.Contains(key, args[0]) && strings.Contains(value, args[1]) || strings.Contains(key, args[0]) && strings.Contains(key, args[1]) {
					fmt.Printf("%s\n%s\n\n", key, value)
				}
			}
		},
