	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) == 1 {
			if args[0] == "servers" || args[0] == "server types" {
				for key := range parseKeyValuePairs(data) {
					...
					serverIP := splitKey[2] // Extract the server IP

					if args[0] == "server types" {
						serverType := splitKey[3] // Extract the server type
		...
		for key, value := range parseKeyValuePairs(data) {
			...
			if !strings.Contains(key, "data") && (strings.Contains(key, args[0]) || strings.Contains(value, args[0])) {
				...
				splitKey := strings.Split(key, "/") // Split key components based on "/"
				serverIP := splitKey[2]             // Extract the server IP
				attribute := splitKey[4]            // Extract the attribute
				// 1/servers2/type3/ip4
				...
				if onlyKeys {
					...
					if len(args) == 1 {
						fmt.Printf("%s:\n%s\n\n", key, value)
					} else if len(args) == 2 && args[1] == "servers" {
						fmt.Println(serverIP)
					} else if len(args) == 2 && args[1] == "server types" {
						fmt.Println(serverIP + "\n" + attribute + ": " + value)
					}
				}
			}
