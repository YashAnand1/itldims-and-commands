...
func listAll(w http.ResponseWriter, r *http.Request) {
	...
	maxRevResponse, err := etcdClient.Get(ctx, "/servers/", clientv3.WithPrefix(), clientv3.WithSort(clientv3.SortByModRevision, clientv3.SortDescend), clientv3.WithLimit(1))
	if err != nil {
	...
	maxRevision := int64(0)
	if len(maxRevResponse.Kvs) > 0 {
		maxRevision = maxRevResponse.Kvs[0].ModRevision
	}

	// Generate the revisions list
	var revisionslist []int64
	for i := maxRevision; i >= maxRevision-5; i-- {
	...
	for _, kv := range response.Kvs {
		revisions, err := getRevisionValues(etcdClient, string(kv.Key), revisionslist)
		if err != nil {
