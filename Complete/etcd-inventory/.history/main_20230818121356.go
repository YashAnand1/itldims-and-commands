...
func listAll(w http.ResponseWriter, r *http.Request) {
	...
	// Get all keys with values and revisions
	response, err := etcdClient.Get(ctx, "/servers/", clientv3.WithPrefix(), clientv3.WithSort(clientv3.SortByModRevision, clientv3.SortDescend))
	if err != nil {
	...
	for _, kv := range response.Kvs {
		...
		// Get revision values similar to the getSpecificKey function
		etcdKeyData := key
		revisions := 0
		revisionsResponse, err := etcdClient.Get(ctx, etcdKeyData, clientv3.WithSort(clientv3.SortByModRevision, clientv3.SortAscend))
		if err != nil {
			log.Printf("Failed to retrieve revisions from etcd: %v", err)
			http.Error(w, "Internal Server Error", http.StatusInternalServerError)
			return
		}

		for _, kv := range revisionsResponse.Kvs {
		...
		revValues, err := getRevisionValues(etcdClient, etcdKeyData, revisionslist)
		if err != nil {
		...
		if len(revValues) > 0 {
			fmt.Fprintf(w, "Previous Value(s):\n")
			for _, revValue := range revValues {
				fmt.Fprintf(w, "Value: %s\n", revValue)
			}
		} else {
			fmt.Fprintf(w, "Previous Value: No previous revision\n")
		}
