...
- itldims get <Server IP> <Server Type> | Display Attribute values of a specific Server
	`,
	Args: cobra.RangeArgs(1, 2), //Here it is defined that 1 or 2 arguments are only allowedpackage main
	...
	- itldims get <Server IP> <Server Type> | Display Attribute values of a specific Server
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if len(args) == 1 && (args[0] == "types") || (args[0] == "servers") {
	...
	func parseKeyValuePairs(data string) map[string]string {
		...
		for _, kv := range keyValuePairs {package main
...
func main() {
	...
	Run: func(cmd *cobra.Command, args []string) { //THe data
		data, err := fetchDataFromAPI()
		if err != nil {
			log.Fatalf("Failed to fetch data from the etcd API: %v", err)
		}
