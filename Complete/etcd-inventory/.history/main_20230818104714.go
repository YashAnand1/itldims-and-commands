...
func listAll(w http.ResponseWriter, r *http.Request) {
	...
	for _, kv := range response.Kvs {
		...
		etcdKeyData := key
		var revisions int
		revisionsResponse, err := etcdClient.Get(ctx, etcdKeyData, clientv3.WithSort(clientv3.SortByCreateRevision, clientv3.SortAscend))

		log.Printf("response %v", revisionsResponse)
		for _, kv := range response.Kvs {
		...
		for i := revisions; i >= revisions-2; i-- {
