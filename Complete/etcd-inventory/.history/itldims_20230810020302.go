...
- itldims get <Server IP> <Server Type> | Display Attribute values of a specific Server
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) == 0 {
			for _, ip := range extractServerIPs(data) {
				fmt.Println(ip)
				fmt.Println("----------------------------")
			}
			return
		...
		serverIP := args[1]
		serverIPKey := "/servers/" + args[1]

		for key, value := range parseKeyValuePairs(data) {
			if strings.Contains(key, args[0]) && strings.Contains(key, serverIPKey) {
				attribute := key[strings.LastIndex(key, "/")+1:]
...
func extractServerIPs(data string) []string {
	var ips []string
	for key := range parseKeyValuePairs(data) {
		if strings.HasPrefix(key, "/servers/Physical/") {
			parts := strings.Split(key, "/")
			if len(parts) >= 5 {
				ips = append(ips, parts[4])
			}
		}
	}
	return ips
