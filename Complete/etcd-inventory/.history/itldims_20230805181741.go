...
var (
	...
	get = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			for key, value := range data {
				if strings.Contains(key, args[0]) && strings.Contains(value, args[0]) && (len(args) == 1 || strings.Contains(value, args[1]) && strings.Contains(key, args[1])) {
					fmt.Printf("%s\n%s\n\n", key, value)
				}
			}
		},
	...
	getx = &cobra.Command{
	Use:   "getx",
	Short: "Get keys with EXACT inputs from etcd API",
	Args:  cobra.ExactArgs(2),
	Run: func(cmd *cobra.Command, args []string) {
		data, err := fetchDataFromEtcdAPI()
		if err != nil {
		...
		for key, value := range data {
			re := regexp.MustCompile(fmt.Sprintf(`\b%s\b`, args[1]))
			if strings.Contains(key, args[0]) && strings.Contains(value, args[0]) && (len(args) == 1 || strings.Contains(value, args[1]) && strings.Contains(key, args[1]) || re.MatchString(value) || re.MatchString(key)) {
				fmt.Printf("%s\n%s\n\n", key, value)
			}
		}
	},
