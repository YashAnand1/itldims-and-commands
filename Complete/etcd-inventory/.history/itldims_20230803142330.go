...
var (
	...
	getCmd = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			// Call the function to fetch and display the filtered keys and their values
			err := fetchAndDisplayKeys(server, attribute)
			if err != nil {
				log.Fatalf("Failed to fetch and display keys: %v", err)
			}
		},
	}
)
...
func fetchAndDisplayKeys(serverIP, attribute string) error {
	// Connect to etcd and get all keys
	response, err := http.Get("http://localhost:8181/servers/")
	if err != nil {
		return fmt.Errorf("failed to connect to the etcd API")
	}
	defer response.Body.Close()
