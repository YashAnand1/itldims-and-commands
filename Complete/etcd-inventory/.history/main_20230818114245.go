...
func listAll(w http.ResponseWriter, r *http.Request) {
	...
	response, err := etcdClient.Get(ctx, "/servers/", clientv3.WithPrefix(), clientv3.WithSort(clientv3.SortByModRevision, clientv3.SortAscend))
	if err != nil {
		log.Printf("%v", err)
	}
	...
	for _, kv := range response.Kvs {
		...
		// Get revisions for the key
		etcdKeyData := string(kv.Key)
		var revisions int

		revisionResponse, err := etcdClient.Get(ctx, etcdKeyData, clientv3.WithSort(clientv3.SortByCreateRevision, clientv3.SortAscend))

		log.Printf("response %v", revisionResponse)

		for _, revKV := range revisionResponse.Kvs {
			revisions = int(revKV.ModRevision)
