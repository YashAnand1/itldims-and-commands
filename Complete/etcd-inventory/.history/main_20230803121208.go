...
func getServerData(w http.ResponseWriter, r *http.Request) {
	...
	key := r.URL.Path[len("/servers/"):] // Remove the leading "/servers/"
...
func getSpecificKey(w http.ResponseWriter, r *http.Request, key string) {
	ctx := context.TODO()
	etcdClient, err := clientv3.New(clientv3.Config{
		Endpoints: []string{etcdHost},
	})
	...
	// Get the value of the specific key
	response, err := etcdClient.Get(ctx, "/"+key) // Add a forward slash at the beginning
	if err != nil {
		log.Printf("Failed to retrieve key from etcd: %v", err)
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
		return
	}

	if len(response.Kvs) > 0 {
		// Only one key-value pair is expected
		kv := response.Kvs[0]
		fmt.Fprintf(w, "Key: %s\n", string(kv.Key))
