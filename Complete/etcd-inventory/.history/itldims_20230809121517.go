...
- itldims get <Server IP> <Server Type> | Displays all Attribute values of a specific Server
	...
	keyOnly bool
	onlykey = &cobra.Command{
		Use:   "onlykey",
		Short: "For displaying only the key without their values.",
		Run: func(cmd *cobra.Command, args []string) {
			keyOnly = truessssssssssssssssssssssssssssssssssssssss
		},
	}
)
...
func init() {
	...
	get.PersistentFlags().BoolVar(&keyOnly, "onlykey", false, "Displays only the keys without their values")
}
